        ...
        <div class="flex flex-col md:flex-row">
            <div class="w-full md:w-1/2 p-8 lg:p-12">
                <div class="max-w-md mx-auto">
                    <div class="text-center md:text-left">
                        ...
                        <h3 class="text-2xl font-bold text-gray-900">Daftar ke Blog Campus</h3>
                        <p class="my-2 text-base text-gray-600">Mulai berbagi ceritamu di Blog Campus</p>
                    </div>

                    <?php Flasher::flash(); ?>

                    <form action="<?= BASEURL; ?>/auth/doRegister" method="POST" class="space-y-4 mt-8"
                          x-data="{ loading: false, showPassword: false, showConfirmPassword: false, refreshCaptcha: false }"
                          @submit="
                              if (loading) { $event.preventDefault(); return false; }
                              ...
                              if (password !== confirmPassword) {
                                  alert('Password tidak cocok!'); $event.preventDefault(); return false;
                              }
                        ...
                        <div class="transform transition duration-300 hover:-translate-y-1">
                            ...
                            <div class="text-xs text-gray-500 mt-1">Min. 8 karakter dengan huruf besar, kecil, dan angka</div>
                        </div>
                        ...
                        <div class="transform transition duration-300 hover:-translate-y-1">
                            ...
                            <div class="mt-1 space-y-3">
                                <div class="flex items-center space-x-3">
                                    <div class="px-4 py-3 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 text-gray-700 font-mono text-lg flex-1">
                                        <div class="flex items-center justify-between">
                                            <?php
                                            try {
                                                echo "<span>" . Captcha::generateCaptcha() . " = ?</span>";
                                            } catch (Exception $e) {
                                                echo "<span>Error generating CAPTCHA</span>";
                                                Logger::error('CAPTCHA generation error: ' . $e->getMessage());
                                            }
                                            ?>
                                            <button type="button" @click="refreshCaptcha = true; setTimeout(() => refreshCaptcha = false, 500); window.location.reload();" class="text-blue-500 hover:text-blue-700 transition-colors duration-200">
                                                <svg :class="{ 'animate-spin': refreshCaptcha }" class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"></path>
                                                </svg>
                                            </button>
                                        </div>
                                    </div>
                                    <input type="number" name="captcha_answer" required placeholder="Jawaban" class="block w-24 px-3 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none transition duration-300 shadow-sm hover:shadow-md">
                                </div>
                                <div class="flex items-center text-xs text-gray-500">
                                    <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"></path>
                                    </svg>
                                    Selesaikan operasi matematika untuk melanjutkan
                                </div>
                            </div>
                        </div>

                        <div class="transform transition duration-300 hover:-translate-y-1">
                            <button type="submit" :disabled="loading" class="w-full flex justify-center py-3 px-4 border-0 rounded-lg text-sm font-medium text-white bg-blue-500 hover:bg-blue-600 disabled:bg-gray-400 transition-all duration-300 shadow-lg hover:shadow-xl outline-none" :class="{ 'opacity-75 cursor-not-allowed': loading }">
                        ...
                        <div class="text-center transform transition duration-300 hover:-translate-y-1">
                            <p class="text-sm text-gray-600">
                                Sudah punya akun?
                                <a href="<?= BASEURL; ?>/auth/login" class="font-medium text-blue-500 hover:text-blue-600 transition-colors duration-300">Masuk disini</a>
                            </p>
                        </div>
                    </form>
                </div>
            ...
            <div class="w-full md:w-1/2 bg-gradient-to-br from-blue-500 to-blue-600 p-8 lg:p-12 flex items-center">
                <div class="max-w-md mx-auto text-center text-white">
                    <h2 class="text-2xl md:text-3xl font-bold mb-4">Bergabung dengan Blog Campus!</h2>
                    <p class="text-base md:text-lg mb-4 md:mb-8 hidden md:block">Mulai berbagi inspirasi dan pengalamanmu bersama komunitas kampus.</p>
                </div>
            </div>
        </div>
    </div>
